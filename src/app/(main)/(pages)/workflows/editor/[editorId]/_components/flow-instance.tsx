...
const FlowInstance = ({
  ...
  const { isStoreLoading, setIsStoreLoading } = useAutoStore();
  ...
  const onFlowAutomation = useCallback(async () => {
    ...
    if (flow) setIsStoreLoading(false);
    toast({
  ...
  return (
    <div className="flex flex-col gap-2 relative">
      {isStoreLoading && (
        <div className="absolute w-full h-full z-50 backdrop-blur-sm flex items-center justify-center">
          <svg
            aria-hidden="true"
      ...
      <div className="flex gap-3 p-4">
        ...
        <Button onClick={startWorkflow} disabled={isFlow.length < 1}>
          Start
        </Button>
      </div>
      {children}
